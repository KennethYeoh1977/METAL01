...
def run_models(data):
    try:
        ...
        data['Zn F/D_MA'] = apply_moving_average(data[target])
        data = data.dropna()
        ...
        st.write("Normalization ranges:")
        for feature in features:
            st.write(f"{feature}: min = {min_values[feature]}, max = {max_values[feature]}")

        scaler = StandardScaler()
        features_with_ma = features + ['Zn F/D_MA']
        ...
        data['Predicted Zn F/D (TDNN)'] = model.predict(data[features_with_ma])
        overall_mae = mean_absolute_error(data[target], data['Predicted Zn F/D (TDNN)'])
        ...
        plt.plot(data['Date'], data['Predicted Zn F/D (TDNN)'], label='Predicted Zn F/D (TDNN)')
        plt.plot(data['Date'], data['Zn F/D'].rolling(window=5).mean(), label='Moving Average (window=5)')
        ...
        plt.text(data['Date'].iloc[int(len(data) * 0.8)], max(data['Zn F/D']), f"MAE: {overall_mae:.2f}", fontsize=12, color='red')
        plt.xlabel('Date')
        plt.ylabel('Zn F/D')
        plt.title('Actual vs Predicted Zn F/D (TDNN) with Moving Average and Next 2 Predictions')
        plt.legend()
